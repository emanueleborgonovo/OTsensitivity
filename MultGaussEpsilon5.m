...
%M=mm;%[Wass2,WassSep,SinkSep,Mean2Sep]=bwsi(X,Y,M,lambda);
     [result]=bigwassersteintestEpsilon2(X,Y,M,2^1,epsilon2);
     results{q}=result;
...
wksname=strcat('WSinkhorn',num2str(epsilon2))
xlswrite(filename,Wsinkhornn,wksname)
...
wksnamet=strcat('T',num2str(epsilon2))
xlswrite(filename,T,wksnamet)
    end
